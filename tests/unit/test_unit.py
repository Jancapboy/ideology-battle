...
class TestUnit:
    def test_unit_alive(self) -> None:
        u = Unit(
            id="test", name="Test", faction_tags=["社会系"], hp=100, max_hp=100, atk=10, spd=1.0
        )
    ...
    def test_unit_heal(self) -> None:
        u = Unit(
            id="test", name="Test", faction_tags=["社会系"], hp=50, max_hp=100, atk=10, spd=1.0
        )
        u.heal(30)
        assert u.hp == 80
        u.heal(100)
    ...
    def test_clone(self) -> None:
        u = Unit(
            id="test", name="Test", faction_tags=["社会系"], hp=80, max_hp=100, atk=10, spd=1.0
        )
        u2 = u.clone()
        assert u2.hp == u.hp
        u2.take_damage(10)
